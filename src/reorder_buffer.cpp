...
reorder_buffer::reorder_buffer(sc_module_name name, unsigned int sz, unsigned int pred_size, unsigned int buffer_size, int flag_mode, nana::listbox &gui, nana::listbox::cat_proxy instr_gui) : sc_module(name),
                                                                                                                                                                                                tam(sz),
                                                                                                                                                                                                flag_mode(flag_mode),
                                                                                                                                                                                                preditor(pred_size),
                                                                                                                                                                                                preditor_mn(pred_size, buffer_size),
...
void reorder_buffer::new_rob_head()
{
    ...
    while (true)
    {
        ...
        case 'B':
            ...
            {
                preditor_mn.mn_update_state(rob_buff[0]->pc, pred, hit);
            }
            else
            {
...
branch_predictor_correlation reorder_buffer::get_preditor_mn()
{
    return preditor_mn;
}
