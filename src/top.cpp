...
void top::rob_modeMN(int n_bits, unsigned int nadd, unsigned int nmul, unsigned int nload, map<string, int> instruct_time, vector<string> instruct_queue, nana::listbox &table, nana::grid &mem_gui, nana::listbox &regs, nana::listbox &instr_gui, nana::label &ccount, nana::listbox &rob_gui, unsigned int m_size)
{
    ...
    mem_slb_bus = unique_ptr<bus>(new bus("mem_slb_bus"));
    iq_rob_bus = unique_ptr<bus>(new bus("iq_rob_bus"));
    rob_statval_bus = unique_ptr<cons_bus_fast>(new cons_bus_fast("rob_statval_bus"));
    rob_adu_bus = unique_ptr<cons_bus>(new cons_bus("rob_adu_bus"));
    inst_bus = unique_ptr<cons_bus>(new cons_bus("inst_bus"));
    rst_bus = unique_ptr<cons_bus>(new cons_bus("rst_bus"));
    sl_bus = unique_ptr<cons_bus>(new cons_bus("sl_bus"));
    rob_bus = unique_ptr<cons_bus>(new cons_bus("rob_bus"));
...
void top::metrics(int cpu_freq, int mode, string bench_name, int n_bits)
{
    ...
    if (fila_r != NULL && rob != NULL)
    {
        ...
        else if (mode == 3)
        {
            ...
            cout << "# Taxa de sucesso - 2 Preditor: " << hit_rate << "%" << endl;
        }
        else
        {
